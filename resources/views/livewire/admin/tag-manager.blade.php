        ...
        @foreach($tags as $tag)
            <div class="bg-white p-4 rounded-lg shadow-sm border border-gray-100 flex items-center justify-between">
                ...
                <div class="flex gap-1">
                    <x-mary-button icon="o-pencil" wire:click="edit({{ $tag->id }})" class="btn-ghost btn-sm btn-circle text-blue-500" />
                    <x-mary-button icon="o-trash" wire:click="delete({{ $tag->id }})" wire:confirm="¿Eliminar etiqueta?" class="btn-ghost btn-sm btn-circle text-red-500" />
                </div>
            </div>
        @endforeach
    </div>
        ...
        <form wire:submit="save" class="space-y-4">
            ...
            <div class="form-control">
                ...
                <div class="flex items-center gap-4">
                    <input type="color" wire:model.live="color" class="p-1 h-10 w-20 block bg-white border border-gray-200 cursor-pointer rounded-lg disabled:opacity-50 disabled:pointer-events-none" title="Elegir color">
                    <div class="px-3 py-1 rounded-full text-white text-sm font-bold shadow-sm" style="background-color: {{ $color }}">
                        Vista Previa
                    </div>
                </div>
            </div>
