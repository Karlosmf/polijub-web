        ...
        @foreach($tags as $tag)
            <div class="bg-white p-4 rounded-lg shadow-sm border border-gray-100 flex items-center justify-between">
                ...
                <div class="flex gap-1">
                    <x-button icon="o-pencil" wire:click="edit({{ $tag->id }})" class="btn-ghost btn-sm btn-circle text-blue-500" />
                    <x-button icon="o-trash" wire:click="delete({{ $tag->id }})" wire:confirm="¿Eliminar etiqueta?" class="btn-ghost btn-sm btn-circle text-red-500" />
                </div>
            </div>
        @endforeach
    </div>
